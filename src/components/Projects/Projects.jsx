  ...
  return (
    <>
      <div className="absolute top-10 bottom-10 left-26 right-16 bg-white/50 p-4 flex gap-2 flex-col rounded-xl">
        ...
        {Tasks.filter((e) => e.designer === props.username).map((e, index) => {
      ...
      {/* Chat btn */}
      <button
        className="absolute  right-4 bg-white/20 rounded-full text-white p-2  top-16 cursor-pointer shadow-lg  duration-200 hover:bg-white/60 z-30"
        onClick={() => {
          {
            {
              mess == false && setMess(true);
              mess == true && setMess(false);
            }
          }
        }}
      >
